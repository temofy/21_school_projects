...
/*   Updated: 2019/02/20 16:43:30 by cheller          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		process_tetriminos(int fd)
{
	...
	if(!(file_str = read_tetriminos(fd)))
		return (-1);
	if ((size = calculate_size(validate(file_str))) == -1)
	{
		ft_putstr("error\n");
		return (-1);
	}
	...
	positions = tet_init_pos(file_str, validate(file_str));
	//print_pos(positions);
	if (validate(file_str) < 4)
